...
class virtual_world:
    ...
    def draw_map(self):
        ...
        for rect in self.map:
            ...
            self.map_mod.append([x1,y1,x2,y2])
            self.canvas.create_rectangle([x1,y1,x2,y2], outline="grey", fill="grey")
            self.canvas.create_rectangle([x1-20 ,y1+20, x2+20, y2-20], outline="black")
            # self.canvas.create_oval(x1,y1,28.2842712475)
            # self.canvas.create_oval(x2,y2,28.2842712475)
            # self.canvas.create_oval(x1,y2,28.2842712475)
            # self.canvas.create_oval(x2,y1,28.2842712475)

            #Draw lines to divide
            # for i in range(len(self.canvas_width)):
    ...
    def get_vrobot_prox(self, side):
        ...
        if (a_r < 0):
            a_r += 6.283
        if (side == "left"):
            a_e = vrobot.a - 3.1415/4 #emitter location 4.0 is exactly at the corner 
        else:
            a_e = vrobot.a + 3.1415/4 #emitter location
        x_e = (vrobot.l-1) * math.sin(a_e) + vrobot.x #emiter pos of left sensor
        y_e = (vrobot.l-1) * math.cos(a_e) + vrobot.y #emiter pos of right sensor
    ...
    def in_collision(self, rectRobot, canvas):
        robotCoords = canvas.coords(rectRobot)
        x1 = robotCoords[0]
        y1 = robotCoords[1]
        x2 = robotCoords[2]
        y2 = robotCoords[3]
        for idx, i in enumerate(robotCoords):
            print "robot ", idx, " ", i
        rectNum = 1

        for rect in self.map_mod:
            # print "rect: ", rect[0], " ", rect[1], " ", ";", " ", rect[2], " ", rect[3]
            if x1 > rect[0] and x1 < rect[2] and y1 < rect[1] and y1 > rect[3]:
                return "cierto"
                return True
            elif x2 > rect[0] and x2 < rect[2] and y2 < rect[1] and y2 > rect[3]:
                return "cierto"
                return True
            rectNum+=1
        rectNum = 0
        ...
        #vrobot = self.vrobot
        # obstales are in self.map

        #intersection = self.radial_intersect(vrobot.a, vrobot.x, vrobot.y)
        #if intersection:
        #    x_i = intersection[0]
        #    y_i = intersection[1]
        #    x_r = vrobot.x
        #    y_r = vrobot.y
        #    dist_to_obs = math.sqrt((y_i-y_r)*(y_i-y_r) + (x_i-x_r)*(x_i-x_r))
        #    if dist_to_obs > 20:
        #        return False
        #    else:
        #        return True
