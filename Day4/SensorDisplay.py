...
def watch(proxQueue, lightQueue):
    ...
    robot = gRobotList[0]
    while True:
        diff = Event("d", abs(robot.getproximity(0)-robot.getproximity(1)))
        floor_l = Event("0", robot.get_floor(0))
        floor_r = Event("1",robot.get_floor(1))
        lightQueue.put(floor_l)
        lightQueue.put(floor_r)
        proxQueue.put(diff)

# def draw(proxQueue,lightQueue):
...
def main(argv=None):
    ...
    proxQueue = Queue.FifoQueue(maxsize=100)
    lightQueue = Queue.FifoQueue(maxsize=100)
    ...
    drawer = threading.Thread(target=draw, args=(proxQueue,lightQueue))
    drawer.start()
