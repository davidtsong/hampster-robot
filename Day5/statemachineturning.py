...
def watch(displayQueue, navQueue):
    ...
    while not gQuit:
        ...
        if prox_l < 50 or prox_r < 50:
            ...
            print "put free"
        if prox_l > 50:
            event = Event("right", [prox_l,prox_r])
            navQueue.put(event)
            print "put right"
        if prox_r > 50:
            event = Event("left",[])
...
def navigate(navQueue):
    ...
    stateLeft = simpleBehavior.add_state("left")
    stateRight = simpleBehavior.add_state("right")
    stateStop = simpleBehavior.add_state("stop")

    simpleBehavior.set_start("free")
    simpleBehavior.set_current("free")

    stateFree.add_transition("left", "left", turnLeft)
    stateFree.add_transition("right", "right", turnRight)
    stateFree.add_transition("free", "free", driveForward)
    stateFree.add_transition("stop", "stop", stop_move)

    stateLeft.add_transition("free", "free", driveForward)
    stateLeft.add_transition("right", "left", turnLeft)
    stateLeft.add_transition("left", "left", turnLeft)
    stateFree.add_transition("stop", "stop", stop_move)

    stateRight.add_transition("free", "free", driveForward)
    stateRight.add_transition("right", "right", turnRight)
    stateRight.add_transition("left", "right", turnRight)
    stateFree.add_transition("stop", "stop", stop_move)
